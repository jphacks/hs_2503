...
let reportMarkers = []; // レポートマーカーを保持（言語切替のため）
...
>>>>>>> 1f75d9a3bfe9c173a2986486e4c02cfee86c043c
  ...
  map = new google.maps.Map(document.getElementById("map"), {
    center: defaultPos,
    zoom: 15,
    mapId: "58be1157ad609efe356c49f6", // 実在のMapID推奨
...
<<<<<<< HEAD
  ...
  if (!userCircle) {
    userCircle = new google.maps.Circle({
      map: mapInstance,
      center: pos,
      radius: pos.accuracy || 50, // 精度(メートル)
      fillColor: "#4285F4",
      fillOpacity: 0.2,
      strokeColor: "#4285F4",
      strokeOpacity: 0.5,
    ...
    <div data-report-id="${id}" class="report-info-window">
      ...
      <div class="evaluation-container" style="display:flex; gap:10px; margin-top: 8px;">
        <div class="like-group">
          <button class="good-btn" onclick="sendEvaluation(${id}, 'good')">👍 役立った</button>
          <span class="count-badge" id="likes-count-${id}">${likesCount || 0}</span>
        </div>
        <div class="dislike-group">
          <button class="bad-btn" onclick="sendEvaluation(${id}, 'bad')">👎 役に立たない</button>
...
window.changeLanguage = function (lang) {
  ...
  // 既存の全AdvancedMarkerElementとCircleをクリアし、参照も破棄
  if (userMarker) {
    userMarker.setMap(null);
    userMarker = null; // ★ 重要：再生成させる
  }
  if (userCircle) {
    userCircle.setMap(null);
    userCircle = null; // ★ 重要：再生成させる
  }
  ...
  // ★ geometry も必要（decodePath を使用）
  script.src = `https://maps.googleapis.com/maps/api/js?key=${window.GOOGLE_MAPS_API_KEY}&language=${lang}&libraries=marker,geometry&callback=initMap`;
  script.async = true;
  script.defer = true;
